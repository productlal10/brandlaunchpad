...
- **Recommended Runtime Split**:
  ...
  - Set `EXTERNAL_API_BASE_URL=https://api.erp.lal10.com` so local `/api/*` routes proxy to the EC2 backend under the isolated `/api/launchpad/*` namespace
  - If you want to override the CSRF helper explicitly, set `EXTERNAL_AUTH_CSRF_URL=https://api.erp.lal10.com/api/launchpad/auth/csrf-token`
